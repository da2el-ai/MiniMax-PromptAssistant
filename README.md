...
6. 気に入った内容は、左カラム上部の**タイトル**を付けて「保存」を押すと残せる
...
- 保存したプロンプトは右カラム下部の「保存プロンプト」に新しい順で並びます。タイトルを押すと
  入力と生成結果がまとめて復元され、`×` で削除できます。「入力をクリア」では消えません
- 保存も localStorage を使うため、ブラウザや端末をまたいだ共有はできません
...
**ComfyUI Context Loop で使うときは参照タグを入れる**

ComfyUI カスタムノード
[Context Loop](https://github.com/ethanfel/ComfyUI-MiniMaxH3-Contex-Loop) の
Scheduled Ref2VA は、シーンごとに参照が入れ替わるため `<Picture N>` の番号が固定できません。
参照アセットの「参照タグ」に `hero_look` のようなエイリアス名を入れると、プロンプト中の
ラベルが `<Picture 1>` ではなく `@hero_look` になります。

- タグは全アセットに入れるか、全アセットで空にするかのどちらかにしてください
- タグを使う場合は 1 アセット = 1 ファイルに分けてください。1 つの参照アセットに
  複数のタグをまとめて書くと、ラベルが 1 つしか振られません
- タグ名は Context Loop 側に登録したエイリアスと綴りを一致させてください。
  一致していないと unknown tag として拒否されます
...
- **コードが決める**: 先頭の指示行、フィールド名、各ショットのカット時刻(`MM:SS.mmm`)、
  話者 ID(`S1`, `S2` …)、参照アセットのラベル(`<Picture N>` または参照タグの `@hero_look`)、
  環境音 / BGM の `N/A`
...
- 参照タグを使ったとき、`<Picture N>` などのラベルに書き換えられていない
